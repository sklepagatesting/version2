    ...
    const completeInterval = setInterval(() => {
      ...
      if (finalProgress >= 100) {
        ...
        setTimeout(() => {
         ...
         heroIn(); // ✅ animate hero
    startObservingText(); // ✅ animate text

    window.dispatchEvent(new Event("scroller:start")); // ✅ 🔥 SCROLLER (carousel) STARTS HERE
  }, 1000);
}
    }, 20);
  });
...
window.addEventListener("resize", () => {
  ...
  if (bp !== prevBreakpoint) {
    sessionStorage.removeItem("hasRunLoader"); // ensure loader plays
    location.reload();
  }
});
